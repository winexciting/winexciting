...
function Footer() {
  ...
  const iscrivi = async () => {
    if (!email.includes("@") || invio) return;
    setInvio(true);
    setErrore(false);
    try {
      const res = await fetch("https://formspree.io/f/xjgnbnbe", {
        method: "POST",
      ...
      <div style={{ maxWidth: 620, margin: "0 auto", textAlign: "center" }}>
        ...
        {inviato ? (
          ...
          <div style={{ display: "flex", gap: 10, maxWidth: 470, margin: "0 auto", flexWrap: "wrap", justifyContent: "center" }}>
            ...
            />
            <button className="btn-lime" onClick={iscrivi} disabled={invio} style={{ opacity: invio ? 0.6 : 1 }}>
              {invio ? "Invio…" : "Iscriviti"}
            </button>
          </div>
        )}
        {errore && (
          <p style={{ fontSize: 13, color: C.bordeaux, fontWeight: 600, marginTop: 14 }}>
            Non siamo riusciti a registrare l'iscrizione. Riprova tra qualche istante.
          </p>
        )}
        <div
          style={{
